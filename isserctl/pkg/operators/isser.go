...
import (
	...
	templates "github.com/codefresh-io/isser/isserctl/templates/kubernetes"
	"k8s.io/apimachinery/pkg/api/errors"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
)

// IsserOperator installs assets on Kubernetes Dind runtimectl Env
type IsserOperator struct {
}

const (
	IsserInstallPattern = ".*.isser.yaml"
)

// Install runtimectl environment
func (u *IsserOperator) Install() error {
	s := store.GetStore()
	...
	for fileName, obj := range kubeObjects {
		match, _ := regexp.MatchString(IsserInstallPattern, fileName)
		if match != true {
			logrus.WithFields(logrus.Fields{
				"Operator": IsserOperatorType,
				"Pattern":  IsserInstallPattern,
...
func (u *IsserOperator) Status() (TableRows, error) {
	s := store.GetStore()
	templatesMap := templates.TemplatesMap()
	kubeObjects, err := KubeObjectsFromTemplates(templatesMap, s.BuildValues())
	if err != nil {
...
func (u *IsserOperator) Delete() error {
	s := store.GetStore()
	templatesMap := templates.TemplatesMap()
	kubeObjects, err := KubeObjectsFromTemplates(templatesMap, s.BuildValues())
	if err != nil {
