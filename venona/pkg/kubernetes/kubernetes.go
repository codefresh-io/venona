...
func (k kube) DeleteResource(ctx context.Context, opts DeleteOptions) error {
	...
	case task.TypeDeletePVC:
		...
		if k.forceDeletePvc {
			_, err := k.client.CoreV1().PersistentVolumeClaims(opts.Namespace).Patch(ctx, opts.Name, types.StrategicMergePatchType, []byte(`{"metadata":{"finalizers":[]}}`), metav1.PatchOptions{})
			if err != nil {
