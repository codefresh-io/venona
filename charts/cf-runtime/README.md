...
| runtime.engine.env.PROPAGATE_ENVS_TO_INTERNAL_CONTAINERS_ENABLE | bool | `true` | If "true", propagate environmental variables from engine to container-logger listed in PROPAGATE_ENVS_TO_INTERNAL_CONTAINERS_LIST |
| runtime.engine.env.PROPAGATE_ENVS_TO_INTERNAL_CONTAINERS_LIST | string | `"CF_TELEMETRY_LOGS_LEVEL,CF_TELEMETRY_OTEL_ENABLE,CF_TELEMETRY_OTEL_ALLOW_HTTP_INSTRUMENTATION,CF_TELEMETRY_PROMETHEUS_ENABLE,CF_TELEMETRY_PROMETHEUS_ENABLE_PROCESS_METRICS,CF_TELEMETRY_PROMETHEUS_HOST,CF_TELEMETRY_PROMETHEUS_PORT,CF_TELEMETRY_PYROSCOPE_ENABLE,PYROSCOPE_SERVER_ADDRESS,LOGGER_LEVEL,LOG_OUTGOING_HTTP_REQUESTS,METRICS_SCRAPE_TIMEOUT_MS,OTEL_EXPORTER_OTLP_ENDPOINT,OTEL_EXPORTER_OTLP_PROTOCOL,OTEL_EXPORTER_OTLP_COMPRESSION,OTEL_LOGS_EXPORTER,OTEL_TRACES_EXPORTER,OTEL_TRACES_SAMPLER,OTEL_METRICS_EXPORTER,OTEL_METRIC_EXPORT_INTERVAL,OTEL_METRIC_EXPORT_TIMEOUT,OTEL_EXPORTER_PROMETHEUS_HOST,OTEL_EXPORTER_PROMETHEUS_PORT,OTEL_SEMCONV_STABILITY_OPT_IN"` | A list of variables to propagate from engine to container-logger (comma-separated string) |
